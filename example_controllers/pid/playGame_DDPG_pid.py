...
with open("example_controllers/pid/configurations.yml", "r") as ymlfile:
    ...
    cfg = yaml.load(ymlfile, Loader=yaml.FullLoader)  # use when you trust the YAML content
...
def playGame(f_diagnostics, train_indicator, port=3101):
    """Training Method."""
    print("TRAIN IS HAPPENING OR NOT",train_indicator)
    ...
    for i in range(episode_count):
        ...
        episode_printer.data['Fuel_start'] = fuel_array[0]
        episode_printer.data['Fuel_end'] = fuel_array[-1]
        episode_printer.data['Fuel_consumed'] = fuel_array[0] - fuel_array[-1]
        episode_printer.data['Reward / Fuel'] = total_reward / (fuel_array[0] - fuel_array[-1])
        ...
        info, running_avg_reward, f_diagnostics,fuel_array, total_reward / (fuel_array[0] - fuel_array[-1]))
...
def document_episode(episode_no, distance_traversed, speed_array,
                     trackPos_array, info, running_avg_reward, f_diagnostics,fuel_array,rew_fuel):
    """
    ...
    f_diagnostics.write(str(fuel_array[0] )+ ",")
    f_diagnostics.write(str(fuel_array[-1])+ ",")
    f_diagnostics.write(str(fuel_array[0] -fuel_array[-1]) + ",")
    f_diagnostics.write(str(rew_fuel) + "\n")
...
if __name__ == "__main__":
    ...
    f_diagnostics = open("stats.txt", 'w')
    f_diagnostics.write("EPISODE,distance_traversed,mean_speed,std_speed,mean_track,std_track,reward_runn,fuel_start,fuel_end,fuel_consumed")
